...
if ace.beats(queen) {
    print ("Ace beats queen!")
} else if queen.beats(ace) {
    print ("Queen beats ace!")
} else {
    print ("Its a tie!")
}
//: - Experiment:
...
let mainten = ServerResponse.maintenance("Back in an hour")
...
switch success {
    ...
    case let .maintenance(maintMessage):
        print("Try later, server said: \(maintMessage)")
}
...
//:
var deck: [Card] = []
for suit: Suit in [ .diamonds, .hearts, .spades, .clubs ] {
    for rank: Rank in [ .ace, .two, .three, .four, .five, .six, .seven, .eight, .nine, .ten, .jack, .queen, .king ] {
